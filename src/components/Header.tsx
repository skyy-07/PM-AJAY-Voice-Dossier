...
export const Header: React.FC<HeaderProps> = ({
  ...
  const channels: NavChannelItem[] = [
    ...
    },
    {
      ...
      description: 'District monitoring, PM-AJAY allocation & escalation console',
      icon: ShieldCheck,
      badge: 'Admin & Audit',
    ...
    <header className={`sticky top-0 z-50 transition-colors duration-200 ${
      ...
      <div className={`${
        ...
        <div className="flex items-center space-x-2 truncate">
          ...
          <span className={`font-medium tracking-wider truncate ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
        ...
        <div className="flex items-center space-x-3 shrink-0">
          ...
          >
            ...
            <span className="hidden sm:inline">{t('header.dossier', selectedLanguage)}</span>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2 flex items-center justify-between gap-2 sm:gap-4">
        ...
        >
          <div className="w-9 h-9 rounded-xl bg-orange-500 text-white flex items-center justify-center shadow-xs transition-transform duration-200 group-hover:scale-105">
            <Mic className="w-4 h-4 sm:w-4.5 sm:h-4.5" />
          </div>

          <div>
            <div className="flex items-center space-x-1.5">
              <h1 className={`font-bold text-sm sm:text-base tracking-tight leading-tight transition-colors ${
                isDark ? 'text-white group-hover:text-orange-200' : 'text-slate-900 group-hover:text-orange-600'
              }`}>
            ...
            <p className={`hidden md:block text-[10px] tracking-normal font-normal ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
        ...
        <div className="relative hidden md:block" ref={channelDropdownRef}>
          ...
          >
            <div className={`p-1 rounded-md ${currentChannel.accentBg} ${currentChannel.accentText}`}>
          ...
          <AnimatePresence>
            {isChannelDropdownOpen && (
              <motion.div
                ...
                className={`absolute left-0 sm:left-1/2 sm:-translate-x-1/2 mt-2 w-72 sm:w-80 rounded-2xl border shadow-xl p-1.5 z-50 overflow-hidden ${
                  isDark 
                    ? 'bg-[#0f172a] border-white/15 text-white shadow-black/60' 
                    : 'bg-white border-slate-200 text-slate-900 shadow-slate-300/60'
                }`}
              >
                <div className={`px-3 py-1.5 text-[10px] font-mono uppercase tracking-wider border-b mb-1 ${
                  ...
                  {channels.map((ch) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-2.5 rounded-xl flex items-start space-x-3 transition-colors cursor-pointer ${
                          isSelected
                            ? isDark ? 'bg-white/10 border border-white/10' : 'bg-orange-50 border border-orange-200/80'
                            : isDark ? 'hover:bg-white/5' : 'hover:bg-slate-100'
                        }`}
                      >
                        <div className={`p-2 rounded-lg shrink-0 mt-0.5 ${ch.accentBg} ${ch.accentText}`}>
                          <Icon className="w-4 h-4" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <span className={`text-xs font-bold tracking-tight ${isSelected ? 'text-orange-500' : ''}`}>
                              {t(ch.labelKey, selectedLanguage)}
                            </span>
                            <span className={`text-[9px] font-mono px-1.5 py-0.5 rounded border ${ch.accentBg} ${ch.accentText} ${ch.accentBorder}`}>
                              {ch.badge}
                            </span>
                          </div>
        ...
        <div className="flex items-center space-x-1 sm:space-x-2">
          ...
          <button
            ...
            title={isSpeaking ? 'Stop voice reading' : 'Read screen aloud (TalkBack)'}
          ...
          <div className={`relative flex items-center border rounded-xl px-2 py-1.5 text-xs transition-colors duration-150 min-h-[36px] ${
            ...
            >
              <option value="hi" className={isDark ? 'bg-[#0b1329] text-white' : 'bg-white text-slate-900'}>हिन्दी</option>
              <option value="bn" className={isDark ? 'bg-[#0b1329] text-white' : 'bg-white text-slate-900'}>বাংলা</option>
              <option value="mr" className={isDark ? 'bg-[#0b1329] text-white' : 'bg-white text-slate-900'}>मराठी</option>
              <option value="ta" className={isDark ? 'bg-[#0b1329] text-white' : 'bg-white text-slate-900'}>தமிழ்</option>
              <option value="te" className={isDark ? 'bg-[#0b1329] text-white' : 'bg-white text-slate-900'}>తెలుగు</option>
              <option value="kn" className={isDark ? 'bg-[#0b1329] text-white' : 'bg-white text-slate-900'}>ಕನ್ನಡ</option>
              <option value="ml" className={isDark ? 'bg-[#0b1329] text-white' : 'bg-white text-slate-900'}>മലയാളം</option>
              <option value="gu" className={isDark ? 'bg-[#0b1329] text-white' : 'bg-white text-slate-900'}>ગુજરાતી</option>
              <option value="pa" className={isDark ? 'bg-[#0b1329] text-white' : 'bg-white text-slate-900'}>ਪੰਜਾਬੀ</option>
              <option value="or" className={isDark ? 'bg-[#0b1329] text-white' : 'bg-white text-slate-900'}>ଓଡ଼ିଆ</option>
              <option value="as" className={isDark ? 'bg-[#0b1329] text-white' : 'bg-white text-slate-900'}>অসমীয়া</option>
            ...
            <AnimatePresence>
              {isToolsDropdownOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 4, scale: 0.97 }}
                  transition={{ duration: 0.15 }}
                  className={`absolute right-0 mt-2 w-64 rounded-2xl border shadow-xl p-3 z-50 space-y-3 ${
                    isDark 
                      ? 'bg-[#0f172a] border-white/15 text-white shadow-black/60' 
                      : 'bg-white border-slate-200 text-slate-900 shadow-slate-300/60'
                  }`}
                >
                  <div className={`text-[10px] font-mono uppercase tracking-wider border-b pb-1.5 ${
                    isDark ? 'text-slate-400 border-white/10' : 'text-slate-500 border-slate-100'
                  }`}>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className={`w-full border rounded-xl px-2.5 py-1.5 text-xs font-medium focus:outline-hidden ${
                        isDark 
                          ? 'bg-[#16203a] border-white/15 text-white' 
                          : 'bg-slate-100 border-slate-200 text-slate-900'
                      }`}
                    >
                      <option value="beneficiary">Beneficiary (Aspirant)</option>
                      <option value="field_worker">Field Facilitator</option>
                      <option value="district_admin">District PM-AJAY Officer</option>
                      <option value="state_admin">State Skill Mission Director</option>
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-between p-2 rounded-xl bg-orange-500/10 border border-orange-500/25 text-orange-500 hover:bg-orange-500/15 text-xs font-semibold transition-colors cursor-pointer"
                  >
                    <span className="flex items-center space-x-1.5">
                      <Sparkles className="w-3.5 h-3.5" />
                      <span>{t('header.dossier', selectedLanguage)}</span>
                    </span>
                    <span className="text-[10px] font-mono font-bold">SAMPLE DATA</span>
                  </button>
                </motion.div>
              )}
            </AnimatePresence>
